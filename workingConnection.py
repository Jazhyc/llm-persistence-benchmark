...
def ssh_connect_and_execute_command(hostname, port, username, key_file_path, command):
    ...
    client.connect(hostname='3.70.158.5', username="ubuntu", pkey=k, allow_agent=False, look_for_keys=False)
...
def evaluate_code(command):
    return ssh_connect_and_execute_command(hostname, port, username, key_file_path, command)


print(evaluate_code("echo 'Hello World'"))
